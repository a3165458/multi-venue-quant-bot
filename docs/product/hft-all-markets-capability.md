...
1. “300 次/秒”是行情事件、订单动作、API 请求，还是实际成交？
2. 账户是 Standard 还是 Premium？当前/计划质押多少 LIT？
3. 初始资金、单日最大可接受损失、单市场和全账户最大敞口是多少？
4. 只做主网永续，还是也包括 Robinhood Chain 和现货？
5. 优先策略是 maker 做市、跨市场套利，还是统计套利？
6. 机器人部署地区及到 Lighter API 的实测 p50/p99 RTT 是多少？
7. 是否允许创建多个 API Key，用不同 key 隔离 maker、cancel 和紧急操作？
